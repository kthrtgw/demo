...
<#list lib.data?values as annotationImportName>
...
<#list lib.data?keys as annotation>
@${annotation}
</#list>
...
<#assign nameLength = simpleName?length>
public class <#if entityPrefix??>${entityPrefix}</#if>${simpleName?substring(0, nameLength - 1)}<#if entitySuffix??>${entitySuffix}</#if><#if superclassSimpleName??> extends ${superclassSimpleName}</#if> {
